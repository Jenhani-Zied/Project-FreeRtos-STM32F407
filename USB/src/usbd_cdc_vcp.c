...
extern int Len1	  ;		  // renvoie longueur string par get_string
 ...
 */
 uint16_t VCP_DataTx(uint8_t* Buf, uint32_t Len) {//static
	uint32_t i = 0;
	while (i < Len) {
...
int VCP_get_string(uint8_t *buf) {
 int i=0;
	...
	*(buf + i+2) = 0;
	...
	Len1=i+2;		   // en commencant par 0 et en comptant les deux caracteres
	APP_tx_ptr_tail += i;
	if (APP_tx_ptr_tail >= APP_TX_BUF_SIZE)
